...
class Round_Func:
    ...
    def subkey_generator(self):
        ...
        c_shift = bin(self.shift_left(int(c_subkey, 2), int(self.__shift_n[self.current_n]), len_subkey))[2:].zfill(
            len_subkey)
        d_shift = bin(self.shift_left(int(d_subkey, 2), int(self.__shift_n[self.current_n]), len_subkey))[2:].zfill(
            len_subkey)
...
print("hiyeonji 이진수(ascii) ::", name)
obj = DES(name, key, 16, pc_1, pc_2, shift_n, s_box, p_box)
print("\n<<암호화 실행>>")
obj.encryption()
print("암호화 결과 :", obj.resulttext)
print("암호화 결과 (str) :", ByteToStr(obj.resulttext))
obj2 = DES(obj.resulttext, key, 16, pc_1, pc_2, shift_n, s_box, p_box)
print("\n<<암호화된 문자를 다시 복호화 실행>>")
obj2.decryption()
print("복호화 결과 :", obj2.resulttext)
print("복호화 결과 (str) :", ByteToStr(obj2.resulttext))